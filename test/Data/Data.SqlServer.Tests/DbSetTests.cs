...
namespace Data.MySql.Tests
{
    public class DbSetTests : DbContextTests
    {
        ...
        [Fact]
        public async void SubWhere()
        {
            var query = _db.Find(m => m.Title.Contains("Core"));

            //子查询
            var subQuery = _db.DbContext.Set<CategoryEntity>()
                .Find(m => m.Name == "ASP.NET Core")
                .Limit(0, 1)
                .Select(m => m.Id);

            query.Where(m => m.CategoryId, QueryOperator.NotEqual, subQuery).ToSql();
        ...
        [Fact]
        public async void SubWhereWithLeftJoin()
        {
            var query = _db.Find(m => m.Title.Contains("Core")).LeftJoin<CategoryEntity>((x, y) => x.CategoryId == y.Id);
            ...
            query.Where((x, y) => x.CategoryId, QueryOperator.NotEqual, subQuery).ToSql();
