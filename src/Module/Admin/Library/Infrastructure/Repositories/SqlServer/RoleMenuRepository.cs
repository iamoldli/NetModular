...
{
    public class RoleMenuRepository : RepositoryAbstract<RoleMenu>, IRoleMenuRepository
    {
        public RoleMenuRepository(IDbContext context) : base(context)
        {
        }
        public Task<IList<RoleMenu>> GetByRoleId(Guid roleId)
        {
            return Db.Find(e => e.RoleId == roleId)
                .LeftJoin<Menu>((x, y) => x.MenuId == y.Id)
