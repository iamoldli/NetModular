...
namespace NetModular.Module.Admin.Application.AuthService.LoginHandler
{
    ...
    public class DefaultUserNameOrEmailLoginHandler : LoginHandlerAbstract, IUserNameOrEmailLoginHandler
    {
        ...
        /// <returns></returns>
        public async Task<LoginResultModel> Handle(UserNameOrEmailLoginModel model)
        {
            var resultModel = new LoginResultModel
            {
                LoginMode = LoginMode.UserNameOrEmail,
        ...
        /// 登录处理
        /// </summary>
        private async Task<IResultModel> Check(UserNameOrEmailLoginModel model, LoginResultModel resultModel)
        {
            var config = _configProvider.Get<AuthConfig>();
            if (!config.LoginMode.UserNameOrEmail)
                return ResultModel.Failed("不允许使用用户名或邮箱的登录方式");
            ...
            //查询账户
            var account = await _repository.GetByUserNameOrEmail(model.UserNameOrEmail, model.AccountType, resultModel.TenantId);
            if (account == null)
