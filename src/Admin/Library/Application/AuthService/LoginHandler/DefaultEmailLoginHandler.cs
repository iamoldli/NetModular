...
namespace NetModular.Module.Admin.Application.AuthService.LoginHandler
{
    ...
    public class DefaultEmailLoginHandler : LoginHandlerAbstract, IEmailLoginHandler
    {
        ...
        /// <returns></returns>
        public async Task<LoginResultModel> Handle(EmailLoginModel model)
        {
        ...
        private async Task<IResultModel> Check(EmailLoginModel model, LoginResultModel resultModel)
        {
            var config = _configProvider.Get<AuthConfig>();
            if (!config.LoginMode.Email)
                return ResultModel.Failed("不允许使用邮箱的登录方式");
            ...
            //查询账户
            var account = await _repository.GetByEmail(model.Email, model.AccountType, resultModel.TenantId);
            if (account == null)
